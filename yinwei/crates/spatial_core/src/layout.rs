...
mod tests {
    ...
    #[test]
    fn enabling_2_0_loads_preset() {
        let mut a = ArrayLayout::default();
        a.speakers[0].az_deg = -90.0;
        a.set_mode(1).unwrap();
        assert_eq!(a.mode, ArrayMode::Stereo2);
        assert!((a.speakers[0].az_deg + 30.0).abs() < 1e-5);
        a.speakers[0].az_deg = -12.0;
        a.set_mode(1).unwrap();
        assert!((a.speakers[0].az_deg + 12.0).abs() < 1e-5);
